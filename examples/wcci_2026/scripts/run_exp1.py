...
def call_with_supported_kwargs(fn, **kwargs):
    """Call fn(**kwargs) but drop kwargs not supported by this installed version."""
...
def main() -> None:
    ...
    ap.add_argument("--config", default="configs/exp1_weather_comfort.json")
    ap.add_argument("--results", default="results/exp1")
    ...
    # Provide a superset of kwargs; unsupported ones are automatically dropped.
    call_with_supported_kwargs(
        fls.generate_fls_one_model,
        results_dir=args.results,
        truth_threshold=args.truth_threshold,
        simplify_fls=True,
        plot_membership_functions=True,

        # Initial outputs (your version clearly supports at least save_initial_fls_txt)
        save_initial_fls_txt=True,
        save_initial_fls_csv=True,
        save_initial_fls_latex=True,

        # Simplified outputs: some versions support these flags, some don't.
        save_simplified_fls_txt=True,
        save_simplified_fls_csv=True,
        save_simplified_fls_latex=True,

        # Graph export (if supported)
